...
from scheduler import MinCapableStepLR
import os
import numpy
import copy
...
class Local_Client:
    # Class defined in original work, minor modifications made
    ...
    def randomize_weights(self):
        ...
        for key, val in self.get_nn_parameters().items():
            if sum([i in key for i in ["num_batches_tracked", "running_mean", "running_var"]]):
                new_weights[key] = copy.deepcopy(val)
            else:
                new_weights[key] = 2.*torch.rand(val.shape) - 1.
        self.update_nn_parameters(new_weights)
    ...
    def train(self, epoch):
        ...
        #Saves train start and end model of a client for that given epoch
        #Returns the running loss

        return running_loss

    #I should insert here getting the gradient: possibly a method, or returned tuple from an existing method
...
class Global_Client:
    '''
    The Global Client class is necessary for when not all parameters are downloaded to local models after aggregation
    This code is adapted from the Local Client class with only minor changes
    '''
    def __init__(self, args, test_data_loader):
        """
        :param args: experiment arguments
        :type args: Arguments
        :param client_idx: Client index
    ...
    def save_model(self, epoch, suffix):
        ...
        Saves the model if necessary.
        """
        self.args.get_logger().debug("Saving global model to flat file storage. Save #{}", epoch)

        if not os.path.exists(self.args.get_save_model_folder_path()):
            os.mkdir(self.args.get_save_model_folder_path())

        full_save_path = os.path.join(self.args.get_save_model_folder_path(), "model_" + "GLOBAL" + "_" + str(epoch) + "_" + suffix + ".model")
        torch.save(self.get_nn_parameters(), full_save_path)
...
def create_clients(args, train_data_loaders, test_data_loader):
    ...
    for idx in range(args.get_num_workers()):
        clients.append(Local_Client(args, idx, train_data_loaders[idx], test_data_loader))
