...
def poison_data(logger, distributed_dataset, num_workers, poisoned_worker_ids, replacement_method, poisoned_class, target):
    ...
    """
    Edited here. We used the label_class_dict instead for logging statistics. Each worker has its own class labels.
    But still we are returning poisoned_dataset same as in the original work.
    """
    label_class_dict = {}
...
def apply_attribute_skew(applied_skewed_propn,
                         aug_list=[A.VerticalFlip(p=0.50), A.HorizontalFlip(p=0.50), A.RandomBrightnessContrast(p=0.0)]):
    """
    Applies horizontal flip or color jitter to a sampled workers' data.

    Inputs:
    applied_skewed_propn - dict: output of apply_skewed_propn
    ...
    """
    workers = applied_skewed_propn.keys()
    len_workers = len(list(workers))
    nsample = random.randint(int(0.25*len_workers), int(0.50*len_workers)) #adj from 0.75 to 0.50
    sampled_workers = random.sample(workers, nsample)

    aug_images = copy.deepcopy(applied_skewed_propn)  # container for augmented images
    for worker in sampled_workers:
        mlabels = applied_skewed_propn[worker].keys()  # labels in workers data
        ...
        #print("L2 Norm:", np.linalg.norm(applied_skewed_propn[worker][label]['X']))
        #print("L2 Norm:", np.linalg.norm(aug_images[worker][label]['X']))
        #print("Diff:", np.linalg.norm(aug_images[worker][label]['X'] - applied_skewed_propn[worker][label]['X']))
    return aug_images
...
def distribute_non_iid_v2(batch_size, train_data_loader, num_workers, poisoned_ids, poisoned_class, n_classes=10):
    train_data = segregate_x_y(train_data_loader, batch_size)
    label_data = segregate_labels(train_data)
    distr_labeldata = distr_labeldata_unequal(label_data, num_workers)
    skewed_label_count = skew_label_count(num_workers, n_classes)
    identified_labels = identify_label_skew(n_classes, skewed_label_count, num_workers, poisoned_ids,
                                               poisoned_class)
    skewed_label_prop = skew_label_proportion(skewed_label_count, identified_labels, num_workers, poisoned_ids,
                                                 poisoned_class)
    applied_skewed_propn = apply_skewed_propn(distr_labeldata, skewed_label_prop)  # Here in this part
    aug_images = apply_attribute_skew(applied_skewed_propn)
    shuffled_data_shards = shuffle_data_shards(aug_images, batch_size)

    #I can probably store aug_images for the Stratified RS
    return shuffled_data_shards
