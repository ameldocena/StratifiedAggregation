...
class StratifiedKrum(MultiKrum):
    ...
    def stratified_aggregate_grads(self, uploaded_grads, strata_weights, global_client=None):
        """
        ...
        for stratum in self.strata:
            ...
            if self.name == "StratKrum":
                ...
                aggregated_grads[stratum] = MultiKrum.aggregate(self, uploaded_grads[stratum])
            elif self.name == "StratTrimMean":
                print("StratTrimMean is selected")
                aggregated_grads[stratum] = TrimmedMean.aggregate(self, uploaded_grads[stratum])
            elif self.name == "StratMedian":
                print("StratMedian is selected")
                aggregated_grads[stratum] = Median.aggregate(self, uploaded_grads[stratum])
            elif self.name == "StratFedAvg":
                print("StratFedAvg is selected")
                uploaded_parameters = uploaded_grads[stratum]
                aggregated_parameters = Aggregator.aggregate(self, uploaded_parameters, global_client)
                aggregated_grads[stratum] = list(aggregated_parameters.values())
        ...
        for layer in range(self.num_layers):
            ...
            for stratum in self.strata:
                component = strata_weights[stratum]*aggregated_grads[stratum][layer]
    ...
    def run_aggregation(self, epoch, stratified_workers, strata_weights, local_clients, global_client):
        ...
        #We have to consider restricted parameter sharing and upload here
        #Both for the upload gradients and parameters
        ...
        if self.name=='StratFedAvg':
            ...
            stratified_aggr_params = self.stratified_aggregate_grads(uploaded_params, strata_weights, global_model)
            # Update params
            stratified_aggr_params = dict(zip(list(global_model.keys()), stratified_aggr_params))
            # Change to FedAvg.update_params to account for restricted sharing
            ...
            stratified_aggr_gradient = self.stratified_aggregate_grads(uploaded_gradients, strata_weights)
            #Model parameters
