...
class StratifiedRandomSampling(SelectionStrategy):
        ...
        for worker in workers:
            ...
            for label in unique_labels:
                ...
                for data_pt in segregated_label[label]['X']:
                    sqrd_diff.append(np.linalg.norm(data_pt - root_disbn[label]))
    ...
    def attribute_skew(self, root_disbn):
        ...
        ave_feature_variation = dict()
        for worker in workers:
            unique_labels = np.unique(self.data_disbn[worker]['Y']) #unique labels in worker data
            ndatapt = len(self.data_disbn[worker]['Y']) #no. of data pts
            attr = list() #list of attribute scores

            for label in unique_labels:
                attr.append(std_dev[worker][label])
            ave_feature_variation[worker] = sum(attr)/ndatapt
        return ave_feature_variation
    ...
    def stratify(self, scores):
        ...
        for score in scores.keys():
            print(list(scores[score].values()))
        ...
        for worker in range(self.num_workers):
            if (scores1[worker] < median_score1):
                ...
                print("worker: {}. score1: {}. S1".format(worker, scores1[worker]))
            else:
                strata['strata2'].append(worker)
                print("worker: {}. score1: {}. S2".format(worker, scores1[worker]))
    ...
    def select_round_workers(self, root_disbn, poisoned_workers, influence=None, worker_disbn = None):
        """
        ...
        if worker_disbn is not None:
            print("Set diff Actual Distr1 vs Strata1:", set(strata['strata1']) - set(worker_disbn['orig']))
            print("Set diff Actual Distr2 vs Strata2:", set(strata['strata2']) - set(worker_disbn['skewed']))
